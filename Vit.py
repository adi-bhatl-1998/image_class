...
image_numpy_array,train_file_data=albumenataions(train_file_data,task="Vit")
...
valid_x=convert_file_name_to_numpy_array(valid_file_data,"Vit")
...
test_x=convert_file_name_to_numpy_array(test_file_data,"Vit")
...
train_x=train_x.reshape(len(train_x),3,224,224)
...
print(valid_x.shape)
valid_x=valid_x.reshape(len(valid_x),3,224,224)
...
vit = vision_transformer.vit_b_16(pretrained=True)
i=0
for params in vit.parameters():
        params.requires_grad=True

for name , params in vit.named_parameters():
    print(f"Layer: {name}, Size: {params.size()}\n\n")
vit.heads[0]=nn.Linear(in_features=768, out_features=3, bias=True)
print(vit.heads[0])
device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
vit = vit.to(device)
...
optimizer=Adam(vit.parameters(),lr=0.009)
...
def train_and_validate( model,loss_criterion,optimizer,epochs=30):
        ...
        for epoch in range(epochs):
            ...
            # Set to evaluation mode
                ...
                torch.save(model, "VIT"+'_model_'+str(epoch)+'.pt')
...
num_epochs =30
trained_model, history, best_epoch = train_and_validate(vit, loss_func, optimizer, num_epochs)
